...
class TestLoadConfig:
    ...
    def test_missing_env_var_raises_value_error(self, config_file: Path) -> None:
        # Omit ALPHA_VANTAGE_API_KEY
        env = {k: v for k, v in MOCK_ENV.items() if k != "ALPHA_VANTAGE_API_KEY"}
        with (
            patch.dict("os.environ", env, clear=True),
            pytest.raises(ValueError, match="ALPHA_VANTAGE_API_KEY"),
        ):
...
class TestLoadSettings:
    ...
    def test_loads_env_from_dotenv_file(self, tmp_path: Path) -> None:
        ...
        env_file.write_text("AV_KEY=from-dotenv\nANT_KEY=ant\nEDGAR_EMAIL=e@e.com\n")
